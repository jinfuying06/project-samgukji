...
- **Breadth expansion started (2026-09-19, D-036), human owner's explicit choice of option 3:** 관도대전 (Battle of Guandu) is now the project's **second event**. Full S1-equivalent pass done: `data/pipelines/build_guandu_sample.py` (D1-equivalent extraction, keyword-scanned across the *entire* raw corpus first), `research/coding_manual_guandu.md` + `research/evidence_matrix_guandu.csv` (R1-equivalent, 25 evidence rows / 9 persons, 3 new labels), `analysis/pipelines/compute_guandu_result.py` → `analysis/outputs/guandu_result.json` (A1-equivalent, schema-validated, 4 divergence cases). Full report: `handoffs/phase_reports/S1_GUANDU_COMPLETION.md`. **No product/backend/frontend work was touched** — the running app still only serves `event.chibi`; this pass was scoped strictly to data-analysis materials per the human owner's own phrasing ("데이터분석 자료 만들기부터"). This event's evidence is a single uncorroborated coding pass, one full validation tier behind 적벽대전's current (human-reviewed) state — see "Committed follow-ups" below.
...
- **관도대전 (D-036) has no second coding pass yet.** Every row in `research/evidence_matrix_guandu.csv` is a single coder's uncorroborated judgment — not even the LLM-LLM double-coding check 적벽대전 got at B1 (κ=0.90), let alone 적벽대전's later human review (D-024). Treat `coding_label` assignments there as provisional. Also uncoded: juan 09 and most of juan 06's non-沮授/田豐 material, out of the 866-paragraph/205-keyword-matched candidate pool 25 rows were drawn from — see `research/coding_manual_guandu.md`'s Coverage note.
...
- **D-036 (2026-09-19): human owner chose option 3 (expand breadth) and 관도대전's data-analysis materials are now done** — see `## Completed` above and `handoffs/phase_reports/S1_GUANDU_COMPLETION.md`. This is data/research-stage only; the app itself has not been touched.

Remaining reasonable next directions, none pre-selected:
1. Treat this as done for now.
2. Continue depth-first: real assistive-technology (screen reader) testing by an actual human — the one accessibility gap no agent can close; resolve the remaining `PARTIAL` rows in `design/accessibility_checklist.md`; actually paste a real `OPENAI_API_KEY` and try the live LLM path end to end; run a second (double-coding) pass on 관도대전's evidence to close its validation gap.
3. Continue expanding breadth: code more of 관도대전's existing candidate pool (juan 09, more of juan 06), or start a third battle, or use more of the already-crawled-but-unused raw corpus for either event.
4. Start deciding how the app layer should support more than one event (`event.chibi` is still hardcoded throughout `app/backend`/`app/frontend`) — a real architecture question nobody has made yet, needed before 관도대전's data can actually be seen in the running app.
5. Move toward an actual S4 release decision for 적벽대전 (would require re-examining the coding-validation gap and real deployment first) — independent of the breadth-expansion track above.
